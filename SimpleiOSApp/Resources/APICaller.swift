...
class APICaller{
    ...
    func getSearchResults(pagination: Bool = false, for queryString: String, completion: @escaping (Result<SearchResults,(Error)>) -> Void){
        ...
        guard let url = URL(string: "\(Constants.baseURL)/search/users?q=\(query)&page=\(self.pageCount)&sort=asc") else{return}
        ...
        let task = URLSession.shared.dataTask(with: URLRequest(url: url)){
                ...
                do{
                    ...
                    self.increasePageCount(for: result.total_count, with: result.items.count)
    ...
    private func increasePageCount(for pageTotal: Int, with itemsPerPage: Int){
        if pageCount < pageTotal / itemsPerPage{
            pageCount += 1
        }
        else{
            isPaginating = false
